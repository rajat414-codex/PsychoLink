...
export default function Auth({ onComplete }) {
  ...
  const sendRealSms = async (phoneNumber, otpCode) => {
    try {
      const targetUrl = `https://www.fast2sms.com/dev/bulkV2?authorization=${FAST2SMS_API_KEY}&route=otp&variables_values=${otpCode}&numbers=${phoneNumber}`;
      // Wrap query in an unrestricted CORS proxy wrapper so requests succeed perfectly in the client browser
      ...
      return { success: false, error: "Network block or local limits" };
    }
  };
  ...
  const sendOtp = async () => {
    ...
    if (result.success) {
      ...
      // Wallet low balance or proxy block warning with code fallback so they can always enter & test

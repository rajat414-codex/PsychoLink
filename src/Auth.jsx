...
// ── HIGH-FIDELITY NAVIGATOR 3D BUBBLE AVATAR COMPONENT ──
function AIBubbleAvatar({ activeAI, expression, isTyping }) {
  // Glow Ring border gradients matching Navigator Bezel aesthetics
  const borderGradient = activeAI === 'AURA'
    ? 'linear-gradient(135deg, #ef4444 0%, #f43f5e 50%, #d946ef 100%)' // Crimson-Fuchsia-Red
    : 'linear-gradient(135deg, #22c55e 0%, #10b981 50%, #06b6d4 100%)'; // Emerald-Cyan-Teal

  const glowColor = activeAI === 'AURA'
    ? 'rgba(239, 68, 68, 0.45)'
    : 'rgba(34, 197, 94, 0.45)';
  ...
  return (
    <div style={{ position: 'relative', width: '220px', height: '220px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      {/* Dynamic ambient float floor shadow */}
      <motion.div 
      ...
      {/* Main 3D Spherical Head Vessel */}
      <div style={{
        width: '100%',
        height: '100%',
        borderRadius: '50%',
        background: 'radial-gradient(circle at 35% 25%, #ffffff 0%, #f8fafc 40%, #e2e8f0 75%, #94a3b8 100%)',
        boxShadow: '0 25px 45px rgba(0,0,0,0.45), inset 0 -12px 20px rgba(148,163,184,0.35), inset 0 8px 12px #ffffff',
        position: 'relative',
        display: 'flex',
        alignItems: 'center',
        ...
        <div style={{
          ...
          padding: '5px',
          boxShadow: `0 0 25px ${glowColor}, inset 0 2px 4px rgba(255,255,255,0.4)`,
          display: 'flex',
          alignItems: 'center',
          justify: 'center',
          position: 'relative',
          ...
          {/* Obsidian dark interior display screen */}
          <div style={{
            width: '100%',
            height: '100%',
            borderRadius: '47px',
            background: 'radial-gradient(circle at center, #111116 0%, #050508 100%)',
            boxShadow: 'inset 0 4px 10px rgba(0,0,0,0.85), inset 0 -2px 6px rgba(255,255,255,0.05)',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            gap: '24px',
            ...
            <div style={{
              ...
              borderRadius: '50% 50% 10% 10% / 80% 80% 10% 10%',
              opacity: 0.8
            }} />

            {/* Glowing emotional vector eyes */}
            <AnimatePresence mode="wait">
              {expression === 'happy' ? (
                // Happy closed-curve squint vectors
                <div style={{ display: 'flex', gap: '24px', position: 'relative', zIndex: 10 }}>
                  <motion.div key="happy-l" initial={{ opacity: 0, scaleY: 0.1 }} animate={{ opacity: 1, scaleY: 1 }} exit={{ opacity: 0 }} style={{ width: '22px', height: '22px' }}>
                    <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
                      <path d="M4 16C6 10 14 10 16 16" stroke="#ffffff" strokeWidth="4.5" strokeLinecap="round" style={{ filter: 'drop-shadow(0 0 6px rgba(255,255,255,0.95))' }}/>
                    </svg>
                  </motion.div>
                  <motion.div key="happy-r" initial={{ opacity: 0, scaleY: 0.1 }} animate={{ opacity: 1, scaleY: 1 }} exit={{ opacity: 0 }} style={{ width: '22px', height: '22px' }}>
                    <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
                      <path d="M4 16C6 10 14 10 16 16" stroke="#ffffff" strokeWidth="4.5" strokeLinecap="round" style={{ filter: 'drop-shadow(0 0 6px rgba(255,255,255,0.95))' }}/>
                    </svg>
                  </motion.div>
                ...
                // Normal high-intensity pill-shaped vector eyes
                <div style={{ display: 'flex', gap: '24px', position: 'relative', zIndex: 10 }}>
                  <motion.div
                    key="normal-l"
                    animate={isTyping ? { scaleY: [1, 0.1, 1], scaleX: [1, 1.2, 1] } : { scaleY: [1, 1, 0.1, 1, 1] }}
                    transition={{
                      duration: isTyping ? 0.4 : 4.5,
                      repeat: Infinity,
                      repeatDelay: isTyping ? 0 : 3.5,
                      ease: 'easeInOut'
                    }}
                    style={{
                      width: '18px',
                      ...
                      boxShadow: '0 0 12px rgba(255,255,255,1), 0 0 4px rgba(255,255,255,0.8)'
                    }}
                  />
                  <motion.div
                    key="normal-r"
...
export default function Auth({ onComplete }) {
  ...
  const speak = (core, text, profile) => {
    ...
    if('speechSynthesis' in window){
      ...
      const estimatedDuration = wordCount * 480; 
  ...
  return (
    <div style={{ minHeight:'100vh', width:'100vw', backgroundColor:'#020308', color:'#fff', fontFamily:J, overflow:'hidden', position:'relative', display:'flex', alignItems:'center', justifyContent:'center' }}>
      ...
      <AnimatePresence mode="wait">
            ...
            >
              ...
              <div style={{
                ...
                <div style={{ marginBottom:'16px' }}>
                  <p style={{ fontSize:'0.63rem', color:'rgba(255,255,255,0.35)', fontFamily:S, letterSpacing:'1.5px', fontWeight:'600', margin:'0 0 6px 4px' }}>EMAIL</p>
        ...
        {/* ═══════════════════════════════════════════
            AI HUB (EMBEDDED NAVIGATOR GLOSSY SPHERE)
        ═══════════════════════════════════════════ */}
        {screen==='aiHub' && (
          <motion.div key="ai-hub" initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ duration:0.8 }}
            ...
            {/* 3D Head Bubble Character Container */}
            ...
            {selectedProfile && (
              <motion.div initial={{ opacity:0,y:8 }} animate={{ opacity:1,y:0 }} transition={{ delay:0.5 }}
                style={{ marginTop:'16px', display:'flex', zIndex:10, alignItems:'center', gap:'9px', background:'rgba(255,255,255,0.05)', backdropFilter:'blur(20px)', border:'1px solid rgba(255,255,255,0.09)', borderRadius:'50px', padding:'6px 14px 6px 7px' }}>
            ...
            {/* Conversation node speech bubble */}
            ...
            <div style={{ display:'flex', gap:'20px', zIndex:20, position:'relative', marginBottom:'8px' }}>
              <span style={{ fontFamily:S, fontSize:'0.7rem', fontWeight:'700', letterSpacing:'1px', color:activeAI==='AURA'?'#ef4444':'rgba(255,255,255,0.2)', transition:'all 0.4s' }}>AURA: {activeAI==='AURA'?'ACTIVE':'STANDBY'}</span>
              <span style={{ color:'rgba(255,255,255,0.12)' }}>·</span>
              <span style={{ fontFamily:S, fontSize:'0.7rem', fontWeight:'700', letterSpacing:'1px', color:activeAI==='MAX'?'#22c55e':'rgba(255,255,255,0.2)', transition:'all 0.4s' }}>MAX: {activeAI==='MAX'?'ACTIVE':'STANDBY'}</span>
            </div>
          </motion.div>
        )}
